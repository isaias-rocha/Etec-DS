...
public class TelaEstoque extends JFrame {
    ...
    public TelaEstoque() {
        ...
        btnSalvar.addActionListener(e->{
            try{Produto p=coletar();
            if(dao.inserir(p)){JOptionPane.showMessageDialog(this,"Salvo!");
            limpar();carregarTabela(dao.listarTodos());}}
            catch(NumberFormatException ex){JOptionPane.showMessageDialog(this,"Preço e Qtd devem ser números.");}});
  ...
  private void carregarTabela(List<Produto> lista){modeloTabela.setRowCount(0);
  for(Produto p:lista)modeloTabela.addRow(new Object[]{p.getId(),p.getNome(),p.getCategoria(),String.format("%.2f",p.getPreco()),p.getQuantidade()});}
